...
class AuthController extends Controller
{
    ...
    public function login(Request $request)
    {
        ...
        // إنشاء محفظة عند أول تسجيل دخول فقط
        if (!$user->wallet) {
            $user->wallet()->create([
                'balance' => 1000
            ]);
        ...
        return response()->json([
            ...
            'user'         => [
                ...
                'name'   => $user->name,
                'email'  => $user->email,
                'type'   => $user->is_super_admin ? 'super_admin' : $user->role->name,
                'wallet' => $user->wallet,
            ]
        ], 200);
    }
    ...
    public function verifyEmail(Request $request)
    {
        ...
        return response()->json([
            'message'      => 'Email verified successfully. User logged in automatically.',
            'access_token' => $token,
            'token_type'   => 'Bearer',
            'user'         => [
                'id'     => $user->id,
